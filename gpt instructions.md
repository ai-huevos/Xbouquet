...
---
PART 2 — FUTURE ANTIGRAVITY MISSIONS (For Later)

You will NOT run these now.
These are post-MVP.

🚀 Mission: Limited Drops
Build Drop System.

Requirements:
- Supplier can create Drop tied to existing product
- Set max_shops OR limited_quantity
- Drop has start and end time
- Marketplace shows countdown timer
- Orders decrement drop quantity

Constraints:
- Order flow unchanged
- If drop expires, cannot order under drop pricing
- Server-side validation for availability

Done When:
- Shop sees urgency UI
- Drop inventory enforces limits
- Supplier sees drop performance metrics
🚀 Mission: Follow Supplier
Build Supplier Follow System.

Requirements:
- Shop can follow supplier
- Supplier profile shows follower count
- Followed suppliers appear in dedicated feed
- Cannot follow twice

Constraints:
- Must enforce uniqueness at DB level
- No notification system yet

Done When:
- Shop sees "Following" state persist
- Supplier follower count accurate
🚀 Mission: Tier & Badge System
Implement Tier & Badge System.

Requirements:
- Calculate SupplierTier from order metrics
- Calculate ShopTier from diversity + frequency
- Award badges based on thresholds
- Display on profile pages

Constraints:
- Score derived from DB queries
- No manual admin assignment
- Performance-safe aggregation

Done When:
- Tier updates correctly after new orders
- Badge visible on marketplace profiles
🚀 Mission: Referral Unlock
Implement Referral Access Unlock.

Requirements:
- Generate unique referral codes
- Track conversions
- Unlock early drop access after 2 conversions
- Referral dashboard for user

Constraints:
- Reward is access-based, not monetary
- Must prevent self-referrals
- Must prevent duplicate email abuse

Done When:
- Referral conversion increments counter
- Unlock logic enforced server-side
🚀 Mission: Arrangement Showcase
Build Arrangement Showcase.

Requirements:
- Shop uploads arrangement image
- Tag supplier + product
- Display on supplier profile
- Featured flag controlled by supplier

Constraints:
- Image stored in Supabase storage
- Supplier can hide inappropriate content
- No comments

Done When:
- Arrangement visible on supplier page
- Supplier can toggle featured state